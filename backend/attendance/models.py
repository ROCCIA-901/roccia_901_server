...
class Attendance(models.Model):
    ...
    week = models.IntegerField(null=True, help_text="주차")
    request_time = models.DateTimeField(null=True, help_text="출석 요청 시간")
    ...
    request_processed_time = models.DateTimeField(null=True, help_text="요청 처리 시간")
...
class AttendanceStats(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="attendance_stats")
    generation = models.ForeignKey(
...
class WeeklyStaffInfo(models.Model):
    ...
    staff = models.ForeignKey(User, on_delete=models.CASCADE, help_text="운영진")
    day_of_week = models.CharField(max_length=10, choices=DAY_OF_WEEK_CHOICES, help_text="요일")
    workout_location = models.CharField(
        max_length=100, choices=WORKOUT_LOCATION_CHOICES, null=True, help_text="운동 지점"
    )
